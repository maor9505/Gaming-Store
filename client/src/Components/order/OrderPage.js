...
export const OrderPage = () => {
  ...
  useEffect(async() => {
    ...
    if (user) {
      if (user.type === "admin") {
        ...
        const res= await axios.get('/order/getOrder',{params:{uid:user.uid,orderId:id}})
  ...
  const CancelOrder = async() => {
     const res= await axios.post('/order/cancelOrder',{order:order})
        ToastAlert("Cancel Order Success");
